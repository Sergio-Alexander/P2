...
void Nova::move_lumens(Nova&& other) noexcept {
    num_lumens = other.num_lumens;
    lumens = other.lumens;
    other.num_lumens = 0;
    other.lumens = nullptr;
...
int Nova::min_glow() const {
    int min_glow_value = INT_MAX;
    for (int i = 0; i < num_lumens; i++) {
        min_glow_value = std::min(min_glow_value, lumens[i]->current_glow_value());
    }
...
int Nova::max_glow() const {
    int max_glow_value = INT_MIN;
    for (int i = 0; i < num_lumens; i++) {
        max_glow_value = std::max(max_glow_value, lumens[i]->current_glow_value());
    }
...
void Nova::recharge_inactive_lumens() {
...
// Post-Condition: The lumen at the given index is reset
void Nova::reset_lumen(int index) {
