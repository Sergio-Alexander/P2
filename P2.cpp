...
int main() {
    ...
    std::cout << "******** Reset all Lumens in all Nova *******" << std::endl;
    reset_all_lumens(novas, num_novas, num_lumens_per_nova);
    std::cout << std::endl;
    display_nova_glows(novas, num_novas);
    std::cout << std::endl;
...
void display_nova_glows(const Nova* const novas[], int num_novas) {
    ...
    for (int i = 0; i < num_novas; ++i) {
        ...
        // Print lumen glow values
        std::cout << "Lumen glow values: ";
        for (int j = 0; j < novas[i]->get_num_lumens(); ++j) {
            std::cout << novas[i]->get_lumen(j)->current_glow_value() << (j < novas[i]->get_num_lumens() - 1 ? ", " : "");
        }
        std::cout << std::endl;

        // Print lumen active status
        std::cout << "Lumen active status: ";
        for (int j = 0; j < novas[i]->get_num_lumens(); ++j) {
            std::cout << (novas[i]->get_lumen(j)->isActive() ? "Active" : "Inactive")
                      << (j < novas[i]->get_num_lumens() - 1 ? ", " : "");
        }
        std::cout << std::endl;
...
void reset_all_lumens(Nova** novas, int num_novas, int num_lumens_per_nova) {
    for (int i = 0; i < num_novas; i++) {
        for (int j = 0; j < num_lumens_per_nova; j++) {
            novas[i]->reset_lumen(j);
        }
    }
}
...
void display_glow_values(const Nova* nova) {
    std::cout << "Lumen glow values: ";
    for (int j = 0; j < nova->get_num_lumens(); ++j) {
        std::cout << nova->get_lumen(j)->current_glow_value() << (j < nova->get_num_lumens() - 1 ? ", " : "");
    }
    std::cout << std::endl;
}
